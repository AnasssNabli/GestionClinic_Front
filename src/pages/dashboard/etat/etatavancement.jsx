...
import { Updateetat } from "./updateetat";
export function Etat() {
  ...
  return (
    <div className="mt-12 mb-8 flex flex-col gap-12">
      <Card>
        ...
        <CardBody className="overflow-x-scroll px-6 pt-0 pb-6">
          <table className="w-full min-w-[640px] table-auto">
            <thead>
              <tr>
                {["Designation", "Ancien taux de realisation", "Nouveau taux de realisation",role === 3 ? "" : "Actions"].map(
                  (el) => (
                    <th
                      key={el}
            ...
            <tbody>
              {currentProjets.map(({ id_projet, designiation, id_type, id_client, mode_facturation, tarif, nbrJh, montant_ht, Duree, description, new_taux_realisation,ancien_taux_realisation, statut }) => (
                <tr key={id_projet}>
                  ...
                  <td className="py-3 px-5 border-b border-blue-gray-50">
                    <div className="w-10/12">
                      ...
                      >
                        {ancien_taux_realisation}%
                      </Typography>
                      <Progress
                        value={ancien_taux_realisation}
                        variant="gradient"
                        color={ancien_taux_realisation === 100 ? "green" : ancien_taux_realisation <= 25? "red":ancien_taux_realisation <=50?"amber":ancien_taux_realisation <=75?"indigo":"teal"}
                        size="md"
                      />
                    </div>
                  </td>
                  ...
                  {role === 3 ? "" :
                    <td className="py-3 px-5 border-b border-blue-gray-50">
                      <Button color="green" size="sm" onClick={() => handleupdateProjectClick({ id_projet, designiation,new_taux_realisation,ancien_taux_realisation })}>Modifier</Button>
                    </td>
                  }
                </tr>
              ))}
      ...
      {isDialogupOpen && (
        <Updateetat  open={isDialogupOpen} project={projettoupdate} setReload={setReload} handleOpen={handleCloseDialogforup} />
      )}
    </div>
  );
}
